...
mclapply(missing, function(substrate) {  
  ...
  # graph
  X = constructGraph(DB, numCPU, Nmin, Nmax, AA)
  GRAPH = X$ALL
  allpos = X$allpos
  ...
  NODE_EMBEDDING = nodeEmbedding(DB, SR1pos, SR2pos, allpos, AAencoding, numCPU)
  # combine with labels
  EMBEDDING = GRAPH %>% dplyr::distinct(substrateID, product, product_detected) %>%
    ...
    as_tibble()
  # FIXME: wb substrate node?
  
  # apply(EMBEDDING,2,function(x) {any(is.na(x))}) %>% any()
...
# ----- OUTPUT -----
fwrite(GRAPH_final, "data/GRAPH_240318_5aa.csv")
fwrite(EMBEDDING_final, "data/EMBEDDING_240318_5aa.csv")
fwrite(INDEX, "data/INDICES_240318_5aa.csv")
