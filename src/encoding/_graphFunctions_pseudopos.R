...
getAllPositions = function(L, Nmin, Nmax, AA, S) {
  ...
  AA4 = outer(as.vector(outer(AA,AA, paste, sep = "")),
              as.vector(outer(AA,AA, paste, sep = "")), paste, sep = "_") %>% as.vector()
  
  SR = str_split_fixed(AA4,"_",Inf)
  AA4 = data.table(aa = AA4, aaP1 = SR[,1], aaP1_ = SR[,2])

  # splice sites: all possible P1-P1' combinations
  P1.P1_ = tidyr::crossing(P1,P1_) %>%
    as_tibble() %>%
    dplyr::mutate(P1.P1_ = paste(P1,P1_, sep = "_"),
                  aaP1 = substr_vec(Spad,P1,P1+1),  # add +1 to coordinates to account for padded sequence
                  aaP1_ = substr_vec(Spad,P1_,P1_+1),  # add +1 to coordinates to account for padded sequence
                  aa = paste(aaP1, aaP1_, sep = "_"))
  
  # filter 4AA combinations for those that are possible in substrate
  AA4 = AA4 %>%
    dplyr::filter(aa %in% P1.P1_$aa)
...
getDetectedPositions = function(DB) {
  ...
  substrateSeq_pad = paste0("X",substrateSeq,"X")
  ...
  psp = DB %>%
    ...
    dplyr::mutate(P1 = str_split_fixed(positions, "_", Inf)[,2] %>% as.numeric(),
                  ...
                  aaP1 = substr_vec(substrateSeq_pad,P1,P1+1),
                  aaP1_ = substr_vec(substrateSeq_pad,P1_,P1_+1),
                  P1.P1_ = paste(P1,P1_, sep = "_")) %>%
    unique()

  # --- amino acids at splice sites
  aa4 = psp %>%
    dplyr::mutate(aa = paste0(substr_vec(substrateSeq_pad,P1,P1+1),"_",substr_vec(substrateSeq_pad,P1_,P1_+1))) %>% # add +1 to coordinates to account for padded sequence
    unique()
...
constructGraph <- function(DB, numCPU, Nmin, Nmax, AA) {
  ...
  p1 = data.frame(P1 = allpos$psp$P1,
    ...
    as_tibble() %>%
    unique() %>% 
    dplyr::mutate(name = paste0("p1_",P1),
                  # FIXME: for benchmarking against NetChop
                  detected = fifelse(P1 %in% detpos$pcp$P1, T, F)) # !!! change to label when PSP is detected
  
  p1_ = data.frame(P1_ = allpos$psp$P1_,
                   aaP1_ = allpos$psp$aaP1_) %>%
    as_tibble() %>%
    unique() %>% 
    dplyr::mutate(name = paste0("p1'_", P1_),
                  # FIXME: for benchmarking against NetChop
                  detected = ifelse(P1_ %in% detpos$pcp$P1_, T, F)) # !!! change to label when PSP is detected
  ...
  COORD_pseudo = mclapply(1:nrow(p1), function(k){
    ...
    return(list(P1 = P1, P1_ = P1_))
  }, 
  mc.preschedule = T,
  mc.cleanup = T,
  mc.cores = numCPU)
  
  allP1 = lapply(COORD_pseudo, function(x){x$P1}) %>%
    data.table::rbindlist() %>%
    na.omit() %>%
    as_tibble() %>%
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! introduce edge weights !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  allP1_ = lapply(COORD_pseudo, function(x){x$P1_}) %>%
    data.table::rbindlist() %>%
    na.omit() %>%
    as_tibble() %>%
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! introduce edge weights !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  source_for_psp = rbindlist(
    list(p1 %>% mutate(P1_ = 0, aaP1_ = NA),
         p1_ %>% mutate(P1 = 0, aaP1 = NA)),
    use.names = TRUE
  )
