...
renv::use_python(type = 'conda', name = 'predictor')
library(stringr)
library(dplyr)
library(data.table)
library(ggplot2)
...
source("src/encoding/_quantitativeFeatures.R")
...
dir.create("data/datasets/polypeptides/",showWarnings = F)
sink("results/constructGraph_polypeptides_log.txt", split = T)

# ----- INPUT -----
load("data/aSPIRE_polypeptides.RData")
Kinetics = Kinetics %>%
  dplyr::select(-substrateID) %>%
  dplyr::rename(substrateID = substrateName)

overview = readxl::read_excel("datasets_polypeptides.xlsx")
...
# get masks
train_subs = overview$substrateName[overview$dataset == "train"] %>% na.omit()
test_subs = overview$substrateName[overview$dataset == "test"] %>% na.omit()
val_subs = overview$substrateName[overview$dataset == "val"] %>% na.omit()
...
for (substrate in substrates$substrateID) {
  ...
  DB = Kinetics %>%
    ...
    dplyr::distinct(substrateID, substrateSeq, pepSeq, productType, spliceType, positions, .keep_all = T) %>%
    dplyr::mutate(multimapper = ifelse(grepl(";",positions), T, F)) # re-think !!!
  ...
  QUANTF = nodeFeatures(DB = DB, tpoints = tpoints)
  EMBEDDING = QUANTF %>%
    right_join(EMBEDDING)
  
  cn = as.character(tpoints)
  EMBEDDING[, cn][is.na(EMBEDDING[, cn])] = 0
  
  # filter for detected
  # !!!!!!!
  GRAPH = GRAPH %>%
    dplyr::filter(educt_detected & product_detected)
  EMBEDDING = EMBEDDING %>%
    dplyr::filter(observed)
  
  # save
  fwrite(GRAPH, file = paste0("data/datasets/polypeptides/",substrate,"_graph.csv"))
  fwrite(EMBEDDING, file = paste0("data/datasets/polypeptides/",substrate,"_embedding.csv"))
}
...
# ----- merge datasets -----
fs_graph = list.files("data/datasets/polypeptides/", pattern = "graph.csv", full.names = T, recursive = T)
...
fs_emb = list.files("data/datasets/polypeptides/", pattern = "_embedding.csv", full.names = T, recursive = T)
...
# there are N/As that are converted to weird stuff!
# GRAPH_final %>% dplyr::filter(is.na(educt_index)) %>% View()
# GRAPH_final = GRAPH_final %>% dplyr::filter(!is.na(educt_index))
nrow(GRAPH_final)


# ----- OUTPUT -----
fwrite(GRAPH_final, "data/POLYPEPTIDE_GRAPH_pseudonodes.csv")
fwrite(EMBEDDING_final, "data/POLYPEPTIDE_EMBEDDING_pseudonodes.csv")
fwrite(INDEX, "data/POLYPEPTIDE_INDICES_pseudonodes.csv")
